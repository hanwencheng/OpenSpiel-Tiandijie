...
from open_spiel.python.games.Tiandijie.primitives.talent.Talent import Talent, TalentTemp


class Talents(Enum):
    # 律: 面圣礼拜: 主动攻击「对战中」物攻提高15%。行动结束时，对十字7格范围2个敌方施加1个随机「有害状态」和「幽禁」状态，持续2回合
    mianshenglibai = TalentTemp(
        "mianshenglibai",
    ...
    shangshandaoxin = TalentTemp(
        ...
        [
            EventListener(
                ...
                partial(TalentEffects.take_effect_of_shangshandaoxin, 1),
            ),
            EventListener(
                EventTypes.skill_end,
                1,
                partial(Rs.self_use_certain_skill, ["guanqie", "shouye"]),
                partial(TalentEffects.take_effect_of_shangshandaoxin, 2),
            )
        ],
    )
    ...
    # 隋酒: 酣淋化愁: 遭受攻击受到伤害时，承受本次伤害量的65%，剩余伤害量将其转化为「贮酿」施加给自身，持续2回合。使用绝学后，行动结束时，为自身和3格范围内的4个其他友方施加「神睿I」状态，持续2回合。
    hanlinhuachou = TalentTemp(
        "hanlinhuachou",
        [ModifierEffect(Rs.always_true, {Ma.damage_container_percentage: 65})],
        [
            EventListener(
                EventTypes.action_end,
                1,
    ...
    # 黎幽: 幽梦冥契: 与携带「有害状态」角色作战时，「对战中」法攻，法防提高15%。行动结束时，对3格范围内随机4个敌方角色施加2个随机「有害状态」。
    youmengminqi = TalentTemp(
        "youmengminqi",
        [
            ModifierEffect(
                partial(Rs.BuffChecks.target_harm_buff_count),
                {Ma.magic_attack_percentage: 15, Ma.magic_defense_percentage: 15},
            ),
        ],
        [
            EventListener(
                ...
                1,
                Rs.always_true,
                partial(TalentEffects.take_effect_of_youmengminqi),
            ),
        ],
    )

    # 于小雪: 自身3格内每存在1名友方，治疗效果提高8 %（最多提高24 %）。受到致命伤害时免除死亡，自身气血恢复100 %，并立刻为周围5格内友方驱散3个「有害状态」，且恢复其气血（恢复量为施术者法攻的1.5倍）。然后化身为女娲石（无法控制），2回合后恢复为人形。（每场战斗最多触发1次）
    cixuewaxin = TalentTemp(
        "cixuewaxin",
        [
            ModifierEffect(
                partial(Rs.PositionChecks.in_range_partner_count_with_limit, 3, 3),
                {Ma.heal_percentage: 8},
            ),
        ],
        [],
    )
